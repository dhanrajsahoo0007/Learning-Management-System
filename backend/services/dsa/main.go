...
import (
	...
	"github.com/dhanrajsahoo0007/Learning-Management-System/backend/services/dsa/handlers"
...
func main() {
	// Load configuration
	cfg, err := config.Load("dsa-service")
	if err != nil {
	...
	log.Println("✅ DSA Service connected to database")
	...
	go func() {
		...
		log.Printf("🚀 DSA Service starting on http://localhost%s", addr)
	...
	log.Println("🛑 Shutting down DSA Service...")
	if err := app.Shutdown(); err != nil {
		log.Fatalf("Server forced to shutdown: %v", err)
	}

	log.Println("✅ DSA Service exited gracefully")
}
