...
import (
	...
	"github.com/dhanrajsahoo0007/Learning-Management-System/backend/services/ai-system-design/handlers"
...
func main() {
	// Load configuration
	cfg, err := config.Load("ai-system-design-service")
	if err != nil {
	...
	log.Println("✅ AI System Design Service connected to database")

	// Initialize repository
	aiSystemDesignRepo := repository.NewAISystemDesignRepository(db)

	// Initialize handler
	aiSystemDesignHandler := handlers.NewAISystemDesignHandler(aiSystemDesignRepo)

	// Create Fiber app
	app := fiber.New(fiber.Config{
		AppName:      "AI System Design Service",
		ServerHeader: "Fiber",
	...
	app.Get("/health", func(c *fiber.Ctx) error {
		...
		return utils.SuccessResponse(c, models.HealthStatus{
			Service:  "ai-system-design-service",
			Status:   "healthy",
			Database: "connected",
			Version:  "1.0.0",
		})
	...
	// AI System Design routes
	app.Get("/topics", aiSystemDesignHandler.GetAll)
	app.Get("/topics/:id", aiSystemDesignHandler.GetByID)

	// Start server in a goroutine
	go func() {
		addr := fmt.Sprintf(":%s", cfg.Service.Port)
		log.Printf("🚀 AI System Design Service starting on http://localhost%s", addr)
	...
	log.Println("🛑 Shutting down AI System Design Service...")
	if err := app.Shutdown(); err != nil {
		log.Fatalf("Server forced to shutdown: %v", err)
	}

	log.Println("✅ AI System Design Service exited gracefully")
}
